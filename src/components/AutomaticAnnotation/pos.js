...
export default function POSAnnotation({ projects, user }) {
  ...
  const [customTags, setCustomTags] = useState([]); // Custom POS tags
  const [tagInput, setTagInput] = useState("");

  const modelOptions = [
    { id: "CAMeL-Lab/bert-base-arabic-camelbert-mix-pos-msa", name: "CAMeL MSA POS" },
    { id: "CAMeL-Lab/bert-base-arabic-camelbert-da-pos-glf", name: "CAMeL DA POS" },
  ];

  // Suggested POS Tags
  const [suggestionTags, setSuggestionTags] = useState([
    "noun_prop", "punc", "noun", "digit", "pron", "adj", "prep", "verb", 
    "conj", "pron_rel", "adj_comp", "adj_num", "adv_rel", "part_neg", 
    "pron_dem", "abbrev", "conj_sub", "part_verb", "noun_num", "noun_quant"
  ]);

  // Fetch custom models if the checkbox is selected
  useEffect(() => {
    if (useCustomModel) {
      AxiosWrapper.get("http://127.0.0.1:8000/list-user-models")
        .then((response) => {
    ...
    try {
      ...
      if (useCustomModel) {
        response = await AxiosWrapper.post(
          `http://127.0.0.1:8000/annotate-with-custom-pos-model/${selectedProject}/${selectedDataSource}/${selectedModel}`,
          payload
        );
      } else {
        response = await AxiosWrapper.post(
          `http://127.0.0.1:8000/POS-annotate/${selectedProject}/${selectedDataSource}`,
          payload
        );
      }

      setStatus(response.data.message || "POS Tagging successful!");
    } catch (err) {
      setStatus("Error during POS tagging: " + (err.response?.data?.message || err.message));
    }
    setIsLoading(false);
  };
  ...
  return (
    <div className="relative">
      ...
      >
        <h3 className="text-xl font-semibold mb-4">POS Annotation Setup</h3>
        <Spacer y={1} />
        ...
        <Select
          fullWidth
          disallowEmptySelection
          label={useCustomModel ? "Select Custom Model..." : "Select POS Model..."}
          selectedKeys={selectedModel ? new Set([selectedModel]) : new Set()}
          onSelectionChange={(e) => setSelectedModel(e.currentKey)}
          placeholder={useCustomModel ? "Select a custom model" : "Select a POS model"}
          className="mb-5"
        >
        ...
        <Input
          ...
          onKeyPress={(e) => e.key === "Enter" && addTag()} // Add tag on Enter key press
...
export async function getServerSideProps(context) {
  ...
  try {
    const projects = (
      await AxiosWrapper.get("http://127.0.0.1:8000/projects", {
    ...
    const projectsWithUsersDataSources = await Promise.all(
      projects.map(async (project) => {
        const userCreatedProject = (
          await AxiosWrapper.get(`http://127.0.0.1:8000/users/${project.created_by_id}`, {
            accessToken: accessToken || ""
          })
        ).data;
        const fileDataSources = (
          await AxiosWrapper.get(`http://127.0.0.1:8000/projects/${project.id}/file-data-sources`, {
            accessToken: accessToken || ""
          })
        ).data;
    ...
    const user = (
      await AxiosWrapper.get("http://127.0.0.1:8000/currentuser", {
        accessToken: accessToken || ""
      })
    ).data;
