    ...
    try {
      ...
      if (useCustomModel) {
        response = await AxiosWrapper.post(
          `http://127.0.0.1:8000/annotate-with-custom-tc-model/${selectedProject}/${selectedDataSource}/${selectedModel}`,
          { job_title: jobTitle, classes }
        );
      } else {
        response = await AxiosWrapper.post(
          `http://127.0.0.1:8000/text-classification/${selectedProject}/${selectedDataSource}`,
          { job_title: jobTitle, classes }
        );
      }
