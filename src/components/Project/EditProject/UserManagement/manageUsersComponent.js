...
export default function ManageUsersComponent({ onClose, projectId, projectCreatedById }) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      setIsLoading(true);
      let allUsers = (await AxiosWrapper.get(`http://50.19.124.30:8000/projects/${projectId}/users`)).data;
      ...
      case "addData":
        return (
          <Switch
            ...
            onValueChange={async (value) => {
              if (user.isMember) {
                (await AxiosWrapper.patch(`http://50.19.124.30:8000/users/${user.id}`, { addData: value }));
                const random = Math.random();
      ...
      case "createJobs":
        return (
          <Switch
            ...
            onValueChange={async (value) => {
              if (user.isMember) {
                (await AxiosWrapper.patch(`http://50.19.124.30:8000/users/${user.id}`, { createJobs: value }));
                const random = Math.random();
