...
export default function AddDataComponent({
  ...
  useEffect(() => {
    const fetchDataSources = async () => {
      setIsLoading(true);
      const dataSources = (await AxiosWrapper.get(`https://takween.ddns.net/projects/${projectId}/file-data-sources`)).data;
  ...
  const handleFileChange = async (e) => {
    try {
      ...
      if (e.target.files.length > 0) {
        ...
        const response = await AxiosWrapper.post(`https://takween.ddns.net/projects/${projectId}/file-data-sources`, formData, {
          headers: {
  ...
  const handleDownload = async (dsId) => {
    setIsLoading(true);
    try {
      const response = (await AxiosWrapper.get(`https://takween.ddns.net/projects/${projectId}/file-data-sources/${dsId}`, {
        responseType: "blob",
      }));
