...
export default function JobPage({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const nextAnnotationData = (await AxiosWrapper.get(`http://50.19.124.30:8000/projects/${projectId}/jobs/${jobId}/annotations?page=${pagination.pageIndex}&itemsPerPage=${pagination.pageSize}&onlyShowUnanotatedData=${onlyShowUnanotatedData}`)).data;
  ...
  const handleExport = async (type, merge = false) => {
    ...
    try {
      let response;
      if (merge) {
        response = await AxiosWrapper.get(`http://50.19.124.30:8000/projects/${projectId}/jobs/${jobId}/annotations/merge/export?type=${type}`, {
          responseType: "blob",
        });
      } else {
        response = await AxiosWrapper.get(`http://50.19.124.30:8000/projects/${projectId}/jobs/${jobId}/annotations/export?type=${type}`, {
          responseType: "blob",
        });
      }
      ...
      <DropdownMenu variant="faded" aria-label="Dropdown menu with description">
        <DropdownSection title="Actions">
          <DropdownItem
            onClick={async () => {
              setIsLoading(true);
              const vizData = (await AxiosWrapper.get(`http://50.19.124.30:8000/projects/${projectId}/jobs/${jobId}/visualization`)).data;
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    const job = (await AxiosWrapper.get(`http://50.19.124.30:8000/projects/${projectId}/jobs/${jobId}`, {
      accessToken: accessToken || "",
    })).data;

    const firstAnnotationDataBatch = (await AxiosWrapper.get(`http://50.19.124.30:8000/projects/${projectId}/jobs/${jobId}/annotations?page=${0}&itemsPerPage=${10}&onlyShowUnanotatedData=${true}`, {
      accessToken: accessToken || "",
    })).data;
